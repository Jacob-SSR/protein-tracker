...
- Sidebar ถาวรฝั่งซ้าย (ยุบเป็นแถบเลื่อนแนวนอนบนจอเล็ก) + แถบบนบอกชื่อหน้า วันที่ และผู้ใช้ที่ล็อกอิน
- หน้าบันทึกอาหารเป็น workspace เดียว: การ์ดสถิติ 3 ใบ · รายการอาหารแยกตามมื้อพร้อมยอดรวมต่อมื้อ ·
  เกจครึ่งวงกลม · ปุ่มลัดอาหารที่พบบ่อย · กราฟ 7 วัน — ใช้ตัวเดียวกันทั้งฝั่งผู้ป่วยและฝั่งเจ้าหน้าที่
- กราฟรายสัปดาห์: ซีรีส์เดียว แยกสีเฉพาะส่วนที่เกินเป้าหมาย (สถานะ ไม่ใช่ซีรีส์ที่สอง)
  มีเส้นเป้าหมายประ ตัวเลขกำกับทุกแท่งแทนแกน Y และ tooltip ตอน hover
  คู่สีผ่าน validator แล้ว (CVD ΔE 8.7 · normal 23.2)
...
| ผู้ป่วย → รายคน | **บันทึกอาหารแทนผู้ป่วย** (การ์ดสถิติ + เกจ + กราฟ 7 วันในหน้าเดียว), บันทึกข้อมูลสุขภาพทั้งชุดด้วยปุ่มเดียว, Preview → Confirm เป้าหมาย, เปิด/ปิดสิทธิ์เข้าระบบ, เก็บเข้าคลัง/ลบถาวร |
...
น้ำหนัก + ผลเลือด (ใส่ได้หลายค่า) + โรคร่วม อยู่ในฟอร์มเดียว **ปุ่มบันทึกปุ่มเดียว**
กดแล้วขึ้นกล่องยืนยันสรุปว่ากำลังจะบันทึกอะไรบ้าง ก่อนเขียนจริง

ทั้งชุดเข้าไปใน **ทรานแซคชันเดียว** (`POST /api/patients/[id]/health-data`) — หนึ่งครั้งที่กดบันทึก
คือการตรวจหนึ่งครั้ง ถ้าส่วนใดพังจะไม่มีอะไรถูกบันทึกเลย ไม่เหลือข้อมูลค้างครึ่งๆ กลางๆ
และลง Audit Log แถวเดียวครบทั้งชุด

## การลบข้อมูลผู้ป่วย

แยกสองระดับโดยตั้งใจ เพราะเป็นข้อมูลสุขภาพ

| ระดับ | ใครทำได้ | เกิดอะไรขึ้น |
|---|---|---|
| เก็บเข้าคลัง | admin | `Patient.isActive = false` ซ่อนจากรายชื่อ กู้คืนได้ ข้อมูลอยู่ครบ |
| ลบถาวร | **SUPER_ADMIN เท่านั้น** | ลบจริงทั้ง cascade ต้องพิมพ์ HN ยืนยันให้ตรงก่อน |

ก่อนลบถาวร ระบบเขียนสรุปจำนวนข้อมูลที่กำลังจะหายลง `AuditLog` ก่อนเสมอ (ถ้าเขียน audit ไม่ผ่าน จะไม่ลบอะไรเลย)
และ `MealItemHistory` ไม่มี FK ผูกกับ Patient จึงยังอยู่ ใช้สอบย้อนหลังได้
